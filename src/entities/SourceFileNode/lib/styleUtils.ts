import type { SourceFileNode } from '../model/types.ts';

export const getNodeBorderColor = (type: SourceFileNode['type']): string => {
  switch (type) {
    // === CALCULATIONS (불변, 청록/파랑 계열) ===
    case 'pure-function':
      return 'border-cyan-500/50 shadow-cyan-900/20'; // 순수 함수
    case 'immutable-data':
      return 'border-blue-500/50 shadow-blue-900/20'; // 불변 상수
    case 'computed':
      return 'border-sky-500/50 shadow-sky-900/20'; // 계산된 값

    // === STATE ACTIONS (상태 변경, 주황/노랑 계열) ===
    case 'ref':
      return 'border-emerald-500/50 shadow-emerald-900/20'; // 기존 ref (하위 호환)
    case 'state-ref':
      return 'border-amber-500/50 shadow-amber-900/20'; // 상태 참조
    case 'state-action':
      return 'border-orange-500/50 shadow-orange-900/20'; // 상태 액션
    case 'mutable-ref':
      return 'border-yellow-500/50 shadow-yellow-900/20'; // 가변 참조

    // === EFFECT ACTIONS (부수효과, 빨강/분홍 계열) ===
    case 'effect-action':
      return 'border-red-500/50 shadow-red-900/20'; // 부수효과 액션
    case 'hook':
      return 'border-violet-500/50 shadow-violet-900/20'; // 커스텀 hook

    // === LEGACY/OTHER ===
    case 'function':
      return 'border-amber-500/50 shadow-amber-900/20'; // 일반 함수
    case 'template':
      return 'border-fuchsia-500/50 shadow-fuchsia-900/20'; // 템플릿/컴포넌트
    case 'call':
      return 'border-yellow-500/50 shadow-yellow-900/20'; // 함수 호출
...
export const getEdgeColor = (type: SourceFileNode['type']): string => {
  switch (type) {
    ...
    // === STATE ACTIONS (상태 변경, 주황/노랑 계열) ===
    case 'ref':
      return '#10b981'; // emerald-500 (하위 호환)
    case 'state-ref':
    ...
    default:
      return '#38bdf8'; // sky-400 (default blue)
  }
};

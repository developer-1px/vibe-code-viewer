...
export const processCodeLines = (
  ...
  if (isTemplate && templateTokenRanges && templateTokenRanges.length > 0) {
    ...
    templateTokenRanges.forEach((range) => {
      const lineInfo = lineOffsets.find((l) => range.startOffset >= l.start && range.startOffset < l.end);
    ...
    return rawLines.map((line, lineIdx) => {
      ...
      tokensOnLine.forEach((range: any) => {
        ...
        const isDirective =
          range.type === 'directive-if' ||
          range.type === 'directive-for' ||
          range.type === 'directive-else' ||
          range.type === 'directive-else-if';

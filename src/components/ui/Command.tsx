...
>((props, ref) => <CommandPrimitive.Empty ref={ref} className="py-6 text-center text-sm text-text-muted" {...props} />);
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Separator ref={ref} className={cn('h-px bg-border-DEFAULT my-1', className)} {...props} />
));
...
const CommandShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
  return <span className={cn('ml-auto text-xs font-mono tracking-normal text-text-muted', className)} {...props} />;
};

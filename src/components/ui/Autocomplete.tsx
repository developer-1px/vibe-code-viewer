        ...
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div
                  className={cn(
                    ...
                    selectedIndex === index ? 'bg-warm-glow/30 text-warm-300' : 'bg-bg-surface',
                    typeColor
                  )}
                >
                  {Icon ? <Icon size={11} strokeWidth={1.5} /> : typeLabel}
                </div>
              ...
              {/* Detail */}
              {item.detail && <span className="shrink-0 text-2xs text-text-muted">{item.detail}</span>}
            </div>
          );
        })}
      </div>

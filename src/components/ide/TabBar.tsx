  ...
  ({ icon: Icon, label, active, dirty, onClose, onClick }, ref) => {
    return (
      <div
        ...
        onClick={onClick}
      >
        {Icon && <Icon size={13} strokeWidth={1.5} className={cn(active ? 'text-text-primary' : 'text-text-muted')} />}
        <span className="flex-1 truncate">{label}</span>
        <div className="flex items-center gap-0.5">
          {dirty && <Indicator variant="warning" className="size-1" />}
          <button
            className={cn('rounded p-0.5 opacity-0 hover:bg-white/10 group-hover:opacity-100', active && 'opacity-100')}
            onClick={(e) => {
              e.stopPropagation();
              onClose?.();
            }}
...
const TabBar = React.forwardRef<HTMLDivElement, TabBarProps>(({ className, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn('flex border-b border-border-DEFAULT bg-bg-base overflow-x-auto', className)}
      {...props}
    >
      {children}
    </div>

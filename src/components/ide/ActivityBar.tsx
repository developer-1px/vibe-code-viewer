    ...
    return (
      ...
      >
        <Icon
          size={18}
          strokeWidth={1.5}
          className={cn('transition-colors', active ? 'text-warm-300' : 'text-text-muted')}
        />
        {hasBadge && <Indicator variant="active" className="absolute right-1 top-1" />}
      </button>
    );
  }
);
...
const ActivityBar = React.forwardRef<HTMLDivElement, ActivityBarProps>(({ className, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
